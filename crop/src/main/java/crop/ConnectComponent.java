...
public class ConnectComponent {
    final int MAX_LABELS = 9000000;
    int next_label = 1;

    public int[] createGrayTotals(BufferedImage image) {
	int height = image.getHeight();
	int width = image.getWidth();
	int[] tblImg = new int[width * height];
	...
	System.out
		.println("From " + next_label + " to " + (j - 1) + " regions");
	next_label = j - 1;
	for (int i = 0; i < image.length; i++)
