...
public class CropWhitespace {
	...
	System.out.println(">>>> trackBadEdges xMargin : " + xMargin
		+ " yMargin : " + yMargin);

	// marginLeft = getBeginMargin(xGrayTotals, xMargin,
	// grayToleranceWidth);
	// marginRight = getEndMargin(xGrayTotals, xMargin, grayToleranceWidth);
	// marginTop = getBeginMargin(yGrayTotals, yMargin,
	// grayToleranceHeight);
	// marginBottom = getEndMargin(yGrayTotals, yMargin,
	// grayToleranceHeight);
     ...
     */
    private void findRectangleCoordinates() {
	...
	int marginHeightToAdd = nbDetectionNoiseHeight / 2;
	int marginWidthToAdd = nbDetectionNoiseWidth / 2;
	...
	System.out.println("nbDetectionNoiseWidth : " + nbDetectionNoiseWidth
		+ " nbDetectionNoiseHeight : " + nbDetectionNoiseHeight
		+ " marginTop : " + marginTop + " marginBottom : "
		+ marginBottom + " marginLeft : " + marginLeft
...
//	    if (xTop - marginHeightToAdd >= 0) {
//		xTop -= marginHeightToAdd;
//	    }
	}
...
//	    if (xBottom + marginHeightToAdd <= height) {
//		xBottom += marginHeightToAdd;
//	    }
	}
...
//	    if (yLeft - marginWidthToAdd >= 0) {
//		yLeft -= marginWidthToAdd;
//	    }
	}
...
//	    if (yRight + marginWidthToAdd <= width) {
//		yLeft += marginWidthToAdd;
//	    }
	}

	System.out.println("Fin find : xTop : " + xTop + " xBottom : "
		+ xBottom + " yLeft : " + yLeft + " yRight : " + yRight
		+ " marginTop : " + marginTop + " marginBottom : "
		+ marginBottom + " marginLeft : " + marginLeft
		+ " marginRight : " + marginRight);
    }
	...
	if (!limiteXTopAtteinte && marginTop <= height) {
	    pourcentageBlancFromXTop = (double) nbRowBlancFromXTop * 100
		    / nbDetectionNoiseHeight;

	    System.out.println("nbRowBlancFromXTop : " + nbRowBlancFromXTop
		    + " -> " + pourcentageBlancFromXTop + "%");

	    if (pourcentageBlancFromXTop > POURCENTAGE_AUTOUR_BLANC) {
		marginTop = marginTop + nbDetectionNoiseHeight;
		System.out.println("nouvelle margin top  : " + marginTop
			+ " xTop : " + xTop);
		checkXTop();
	    }
	}
	System.out.println(">>>> fin checkTop : marginTop : " + marginTop
		+ " xTop : " + xTop);
    }
	...
	if (!limiteXBottomAtteinte && marginBottom >= 0) {
	    pourcentageBlancFromXBottom = (double) nbRowBlancFromXBottom * 100
		    / nbDetectionNoiseHeight;

	    System.out.println("nbRowBlancFromXBottom : "
		    + nbRowBlancFromXBottom + " -> "
		    + pourcentageBlancFromXBottom + "%");

	    if (pourcentageBlancFromXBottom > POURCENTAGE_AUTOUR_BLANC) {
		marginBottom = marginBottom - nbDetectionNoiseHeight;
		System.out.println("nouvelle margin bottom  : " + marginBottom
			+ " xBottom : " + xBottom);
		checkXBottom();
	...
	if (!limiteYLeftAtteinte && marginLeft <= width) {
	    pourcentageBlancFromYLeft = (double) nbRowBlancFromYLeft * 100
		    / nbDetectionNoiseWidth;

	    System.out.println("nbRowBlancFromYLeft : " + nbRowBlancFromYLeft
		    + " -> " + pourcentageBlancFromYLeft + "%");

	    if (pourcentageBlancFromYLeft > POURCENTAGE_AUTOUR_BLANC) {
		marginLeft = marginLeft + nbDetectionNoiseWidth;
		System.out.println("nouvelle margin left  : " + marginLeft
			+ " yLeft : " + yLeft);
		checkYLeft();
	    }
	}
	...
	if (!limiteYRightAtteinte && marginRight >= 0) {
	    pourcentageBlancFromYRight = (double) nbRowBlancFromYRight * 100
		    / nbDetectionNoiseWidth;

	    System.out.println("nbRowBlancFromYRight : " + nbRowBlancFromYRight
		    + " -> " + pourcentageBlancFromYRight + "%");

	    if (pourcentageBlancFromYRight > POURCENTAGE_AUTOUR_BLANC) {
		marginRight = marginRight - nbDetectionNoiseWidth;
		System.out.println("nouvelle margin right  : " + marginRight
			+ " yRight : " + yRight);
		checkYRight();
	    }
	}
    ...
    /**
     * er a If the edges of the document have bad (dark) edges, try to find
     * (within the <code>marginLimit</code>) from where the document is
     * whitespace.
     * <p>
	...
	for (int i = 0; i < marginLimit; i++) {
	    System.out.println(">>> getBeginMargin grayTotals[" + i + "] : "
		    + grayTotals[i]);
	    if (grayTotals[i] < grayTolerance) {
		count++;
		System.out.println(">>> getBeginMargin count : " + count);
	...
	System.out.println(">>>> cropImage >>> xTop : " + xTop + " xBottom : "
		+ xBottom + " yRight : " + yRight + " yLeft : " + yLeft);
	if (xTop == Integer.MAX_VALUE || yLeft == Integer.MAX_VALUE) {
	    ...
	    rr = rr.createWritableChild(yLeft, xTop, newWidth, newHeight, 0, 0,
		    null);
