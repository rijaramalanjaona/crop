...
public class CropWhitespace {
	...
	private static final int POURCENTAGE_DETECTION_NOISE = 5;
	private static final int POURCENTAGE_AUTOUR_BLANC = 85;
	...
	private int iterationFindRectangleCoordinates;
	...
	private void findRectangleCoordinates() {
		iterationFindRectangleCoordinates++;
		System.out.println(">>>>>>>> iterationFindRectangleCoordinates : " + iterationFindRectangleCoordinates);
		...
		// autour
		System.out.println("check xBottom");

		for (int k = xBottom; k >= xBottom - nbDetectionNoiseHeight; k--) {
			System.out.println("xGrayTotals[xBottom - " + k + "] : " + xGrayTotals[k]);
		...
		System.out.println("nbRowBlancFromXBottom : " + nbRowBlancFromXBottom + " -> " + pourcentageBlancFromXBottom
				+ "%");

		if (pourcentageBlancFromXBottom > POURCENTAGE_AUTOUR_BLANC) {
			for (int k = xBottom; k >= xBottom - nbDetectionNoiseHeight; k--) {
				xGrayTotals[k] = 255 * width;
				System.out.println(">>> Blanc xGrayTotals[" + k + "] : " + xGrayTotals[k]);
			}
			xBottom = xBottom - nbDetectionNoiseHeight;
			findRectangleCoordinates();
		}
	 ...
	 * @return
	 */
	private int getBeginMargin(int[] grayTotals, int marginLimit, int grayTolerance) {
		boolean foundGrayIndex = false;
		int firstGrayIndex = 0;

		int count = 0;
		for (int i = 0; i < marginLimit; i++) {
			System.out.println(">>> getBeginMargin grayTotals[" + i + "] : " + grayTotals[i]);
	 ...
	 * @return
	 */
	private int getEndMargin(int[] grayTotals, int marginLimit, int grayTolerance) {
	 ...
	 *             image.
	 */
	private void checkArgs(int width, int height, int top, int right, int bottom, int left)
			throws IllegalArgumentException {
