...
public class Corners {
	...
	for (int x = 0; x < width; x++) {
	    ...
	    xBottomRow[x] = new GrayColor(image.getRGB(x, lastRow))
		    .getGrayscale();
	}
	...
	for (int y = 0; y < height; y++) {
	    ...
	    yRightCol[y] = new GrayColor(image.getRGB(lastCol, y))
		    .getGrayscale();
	}
    }
	...
	for (int leftTotal = 0, rightTotal = 0; !equalTotals && length > 1; leftTotal = 0, rightTotal = 0) {
	    System.out
		    .format("length=%d, leftIndexBegin=%d, leftIndexEnd=%d, rightIndexBegin=%d, rightIndexEnd=%d%n",
			    length, leftIndexBegin, leftIndexEnd,
			    rightIndexBegin, rightIndexEnd);
		...
		if (leftTotal < rightTotal) { // left half is darker
		    System.out.format("Left is darker : %d < %d%n", leftTotal,
			    rightTotal);
		    ...
		    System.out.format("Right is darker : %d > %d%n", leftTotal,
			    rightTotal);
		...
		System.out.format("Targetting a corner : %d, %d%n", leftTotal,
			rightTotal);
		for (int i = leftIndexBegin; i < rightIndexEnd; i++) {
		    System.out
			    .print(StringUtils.leftPad("" + array[i], 3, ' '));
		    System.out.print(", ");
		}
		System.out.println();
	...
	} else if (isLowest(leftDistance, bottomDistance, rightDistance,
		topDistance)) {
	    bestCorner = BestCorner.LEFT;

	} else if (isLowest(bottomDistance, rightDistance, topDistance,
		leftDistance)) {
	    bestCorner = BestCorner.BOTTOM;

	} else {
	    assert isLowest(rightDistance, topDistance, leftDistance,
		    bottomDistance);
